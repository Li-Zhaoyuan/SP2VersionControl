...
#include "LoadTGA.h"
//#include "LoadOBJ.h"
SP2Framework::SP2Framework()
{
}
SP2Framework::~SP2Framework()
{
}
void SP2Framework::Init()
{
...
void SP2Framework::Update(double dt)
{
	camera.Update(dt, 15);
	fps = 1 / dt;
...
void SP2Framework::renderMesh(Mesh *mesh, bool enableLight)
{
...
void SP2Framework::RenderText(Mesh* mesh, std::string text, Color color)
{
...
void SP2Framework::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
	if (!mesh || mesh->textureID <= 0) //Proper error check
		return;
...
void SP2Framework::renderOBJonScreen()
{
	glDisable(GL_DEPTH_TEST);
	Mtx44 ortho;
	ortho.SetToOrtho(0, 80, 0, 60, -10, 10); //size of screen UI
...
void SP2Framework::RenderSkybox()
{
	modelStack.PushMatrix();
...
void SP2Framework::collision(float dt)
{
...
void SP2Framework::RenderTextCompiled()
{
	
	std::stringstream playerPos;
	playerPos << "X = " << camPosX << " Y = " << camPosY << " Z = " << camPosz;
	RenderTextOnScreen(meshList[GEO_TEXT], playerPos.str(), Color(1, 0, 0), 3, 7, 19);
	std::stringstream ss;
	ss << "FPS:" << fps;
	RenderTextOnScreen(meshList[GEO_TEXT], ss.str(), Color(0, 1, 0), 3, 0, 19);
	RenderTextOnScreen(meshList[GEO_TEXT], "+", Color(1, 0, 0), 3, 13, 9.5);
